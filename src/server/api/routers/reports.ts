...
export const reportsRouter = router({
  ...
  getDashboardMetrics: protectedProcedure.query(async ({ ctx }) => {
    ...
    const appointmentsToday = await ctx.db.select().from(appointments).where(and(eq(appointments.organizationId, ctx.orgId), gte(appointments.appointmentDate, startDay)));
