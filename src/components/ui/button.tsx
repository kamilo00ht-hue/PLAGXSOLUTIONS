...
type ButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'ghost' | 'violet' };
...
export function Button({ className, variant = 'primary', ...props }: ButtonProps) {
  ...
  return (
    <button
      className={cn('rounded-xl border px-4 py-2 text-sm font-semibold transition-all neon-glow', styles[variant], className)}
      {...props}
    />
  );
}
